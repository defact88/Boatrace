...
# ------ 永続化対象の賭式(TT/FF は合意通り除外) ------
BET_TYPES_PERSISTED = ("3T", "3F", "2T", "2F", "KK")

# 全組み合わせを記録する最低間隔(秒)。これより短い間隔で_exec_oddsが
# 呼ばれても、フルスナップショットはこの間隔でのみ書き込む(間引き)
FULL_SNAPSHOT_INTERVAL_SEC = 300


# =====================================================================
# ① 予測エンジンとの接続インターフェース(プレースホルダ)
# =====================================================================
...
def _parse_odds_value(raw:str):
    """"12.3" / "12.3-15.6" / "欠場" / "" を (odds:float|None, is_absent:int) に変換"""
...
# 永続化(ハイブリッド保存方針)
# =====================================================================
def persist_odds_snapshot(conn:sqlite3.Connection, data:dict, d:date, venue_id:int, race_no:int,
                          hits:list, is_final:bool, do_full_snapshot:bool) -> int:
    """
    合意済みのハイブリッド方針:
      ① is_final=True         : 全組み合わせを必ず保存(学習・決済検証用、無期限保持)
      ② hits に含まれる組合せ : 判定した瞬間の詳細を保存(閾値判断の根拠として)
      ③ do_full_snapshot=True : 全組み合わせを保存(間引き済み、パージ対象)
      TT/FF は BET_TYPES_PERSISTED に含めないため保存されない
    """
    now      = dt.now()
    ...
    for bet_type in BET_TYPES_PERSISTED:
        for key, raw in data.get(bet_type, {}).items():
            ...
            should_write = is_final or do_full_snapshot or (bet_type, key) in hit_keys
    ...
    with conn:
        conn.executemany("""
            INSERT INTO Odds_snapshots(
                race_id, date, venue_id, race_no, bet_type,
